...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    q_check = Quaternion(*q_prev).quat_mult_left(Quaternion(axis_angle=angle_normalize(imu_w.data[k-1] * delta_t)))

    # 1.1 Linearize the motion model and compute Jacobians
    F = np.identity(9)
    F[0:3, 3:6] = np.identity(3) * delta_t
    F[3:6, 6:9] = -skew_symmetric(C_ns @ (imu_f.data[k-1].reshape(3,1))) * delta_t
    ...
    if lidar_i < lidar.data.shape[0] and imu_f.t[k] >= lidar.t[lidar_i]:
        p_check, v_check, q_check, p_cov_check = measurement_update(
            var_lidar, p_cov_check, lidar.data[lidar_i], p_check, v_check, q_check
        )
        lidar_i += 1
